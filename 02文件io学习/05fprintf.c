...
int main()
{
    ...
    /**
     * @brief 向文件写入格式化字符串，读写权限记录在fopen时的模式中
     * FILE *stream：文件指针，指向要写入的文件。
     * const char *__restrict __format：格式字符串。
     * ...：可变参数列表，根据格式字符串提供的格式依次传入相应的参数。
     * @returns：成功时返回写入的字符数，失败时返回负值。
     * int fprintf (FILE *__restrict __stream,
            const char *__restrict __format, ...)
     */
    int fprintf_result = fprintf(ioFile, "I am %d years old.\n", 18); //向文件写入格式化字符串
    if(fprintf_result == EOF) {
        printf("failed 写入格式化字符串失败\n");
    }
    else {
        printf("succeed 写入格式化字符串%d成功\n", fprintf_result);
    }
